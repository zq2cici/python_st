...
def push_wechat(name, real_price, real_percent, type):
    wei_name=u'Z-Q-ZHANG'
    print(wei_name)
    itchat.auto_login(hotReload=True)
    print('have logined weixin')
    account=itchat.get_friends(wei_name)
    print('weiweiweiweiwei')
    print(account)
    for i in account:
        if i[u'PYQuanPin']==wei_name:
            toName=i['UserName']
...
def main():
    ...
    if str(choice) == '1':
        ...
        stock_lists_price = read_stock('p.txt')
        #while 1:
        t = 0
        for each_stock in stock_lists_price:
            ...
            t = meet_price(code, price_up, price_down,'wechat')
            if t:
